# this script is to summarize the kingdom level models
...
# first it mimics the 'kingdom_level_modelling.R' script 
# in kingdom to see how many possible models could have been fit
...
kingdom_summary <- readRDS("analysis_data/summary_kingdom.RDS")

# read in prepared data for analysis
analysis_data2 <- readRDS("analysis_data/final_data_for_analysis_kingdom.RDS")

# read in list of files for the modelling results
model_fit_files <- list.files("model_objects/kingdom_level")
...
# did each potential kingdom write out a model object?
nrow(kingdom_summary)==length(model_fit_files_ghm)
nrow(kingdom_summary)==length(model_fit_files_viirs)

# write a function that
# reads in each model fit, one at a time
# and then summarizes the information
# in that model fit
summarize_kingdom_models_function <- function(kingdom_name){
  
  message(paste0("Summarizing ", stringr::word(kingdom_name, 1, sep="_")))
  
  # get original data used for modelling
  # filter to data
  dat <- analysis_data2 %>%
    dplyr::filter(kingdom==stringr::word(kingdom_name, 1, sep="_")) %>%
  ...
  ggplot(dat, aes(x=body_size_scaled_log10, y=mean_urban_score_ghm, color=metadata))+
    geom_point()+
    theme_bw()+
    theme(axis.text=element_text(color="black"))
  
  mod <- readRDS(paste0("model_objects/kingdom_level/", kingdom_name))
  ...
  out_df <- draws %>%
    ...
    mutate(kingdom=stringr::word(kingdom_name, 1, sep="_")) %>%
    ungroup()
...
# first for ghm
kingdom_results_ghm <- bind_rows(lapply_with_error(model_fit_files_ghm, summarize_kingdom_models_function)) %>%
  mutate(response_type="ghm")

# then for viirs
kingdom_results_viirs <- bind_rows(lapply_with_error(model_fit_files_viirs, summarize_kingdom_models_function)) %>%
  mutate(response_type="viirs")

#combine
kingdom_overall_summary <- kingdom_results_ghm %>%
  bind_rows(kingdom_results_viirs) %>%
  left_join(., kingdom_summary, by="kingdom")

saveRDS(kingdom_overall_summary, "intermediate_results/kingdom_level_results.RDS")

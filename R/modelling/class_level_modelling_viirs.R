# this script is fitting models at the class level
# where the urban scores were calculated using the viirs night-time lights
# it mirrors the script "R/modelling/class_level_modelling_viirs.R"
...
# first calculate the number of classs with >2 species in a subrealm
# this seems like a logical cutoff in that we want at least 2 species in a subrealm fo
# form a slope
class_subrealm <- analysis_data %>%
  group_by(class, subrealm) %>%
  summarize(number_species_sub_class=n()) %>%
  dplyr::filter(number_species_sub_class>=2)
...
body_size_summarized <- analysis_data %>%
  ungroup() %>%
  unite(class_sub, class, subrealm, sep="_", remove=FALSE) %>%
  dplyr::filter(class_sub %in% local(class_subrealm %>%
                                       unite(class_sub, class, subrealm, sep="_", remove=FALSE) %>%
                                       .$class_sub)) %>%
  unite(metadata2, type, units, measurement_detail, sep="_", remove=FALSE) %>%
  group_by(class, metadata2) %>%
  summarize(N=n())
...
# now summarize the data at the class level
# first filter out based on the data above regarding 2 species in  class subrealm
# number of classes with >10 species
...
saveRDS(class_summary, "analysis_data/summary_class.RDS")
...
saveRDS(analysis_data2, "analysis_data/final_data_for_analysis_class.RDS")

analysis_data2 %>%
  group_by(metadata) %>%
  summarize(N=n()) %>%
  mutate(model_type="class_viirs") %>%
  saveRDS(., "intermediate_results/model_metadata_usage_summary/metadata_summary_class_viirs.RDS")

# now a function to model every class possible
# and save the model results out
# as a file
class_level_modelling_function <- function(class_name){
  
  message(paste0("Modelling ", class_name))
  
  # filter to data
  dat <- analysis_data2 %>%
    dplyr::filter(class==class_name) %>%
    left_join(., class_summary %>%
                dplyr::select(class, model_type) %>%
                distinct()) %>%
    ungroup() %>%
    group_by(metadata2) %>%
  ...
  if(unique(dat$model_type)=="type 1"){
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm) + (1|metadata2),
              ...
              file=paste0("model_objects/class_level/", class_name, "_viirs_type_1"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10,
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/class_level/", class_name, "_viirs_type_2"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/class_level/", class_name, "_viirs_type_3"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1|metadata2),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/class_level/", class_name, "_viirs_type_4"))
    
  }
}

# now apply this modelling function over every class
# but will use an lapply with error
lapply_with_error(class_summary$class, class_level_modelling_function)

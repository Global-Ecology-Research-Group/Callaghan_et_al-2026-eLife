...
model_fit_files <- list.files("model_objects/family_level")
...
summarize_family_models_function <- function(family_name){
  ...
  mod <- readRDS(paste0("model_objects/family_level/", family_name))
...
# first for ghm
family_results_ghm <- bind_rows(lapply_with_error(model_fit_files_ghm, summarize_family_models_function)) %>%
...
#combine
family_overall_summary <- family_results_ghm %>%
  bind_rows(family_results_viirs) %>%
  left_join(., family_summary, by="family")

saveRDS(family_overall_summary, "intermediate_results/family_level_results.RDS")

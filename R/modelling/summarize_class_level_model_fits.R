# this script is to summarize the class level models
...
# first it mimics the 'class_level_modelling.R' script 
# in class to see how many possible models could have been fit
...
class_summary <- readRDS("analysis_data/summary_class.RDS")
...
model_fit_files <- list.files("model_objects/class_level")
...
# did each potential class write out a model object?
nrow(class_summary)==length(model_fit_files_ghm)
nrow(class_summary)==length(model_fit_files_viirs)

# write a function that
# reads in each model fit, one at a time
# and then summarizes the information
# in that model fit
summarize_class_models_function <- function(class_name){
  
  message(paste0("Summarizing ", stringr::word(class_name, 1, sep="_")))
  
  # get original data used for modelling
  # filter to data
  dat <- analysis_data2 %>%
    dplyr::filter(class==stringr::word(class_name, 1, sep="_")) %>%
  ...
  mod <- readRDS(paste0("model_objects/class_level/", class_name))
  ...
  out_df <- draws %>%
    ...
    mutate(class=stringr::word(class_name, 1, sep="_")) %>%
    ungroup()
...
# first for ghm
class_results_ghm <- bind_rows(lapply_with_error(model_fit_files_ghm, summarize_class_models_function)) %>%
  mutate(response_type="ghm")

# then for viirs
class_results_viirs <- bind_rows(lapply_with_error(model_fit_files_viirs, summarize_class_models_function)) %>%
  mutate(response_type="viirs")

#combine
class_overall_summary <- class_results_ghm %>%
  bind_rows(class_results_viirs) %>%
  left_join(., class_summary, by="class")

saveRDS(class_overall_summary, "intermediate_results/class_level_results.RDS")

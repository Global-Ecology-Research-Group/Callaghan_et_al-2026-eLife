# this script is fitting models at the order level
# where the urban scores were calculated using the viirs night-time lights
# it mirrors the script "R/modelling/order_level_modelling_ghm.R"
...
# first calculate the number of orders with >2 species in a subrealm
# this seems like a logical cutoff in that we want at least 2 species in a subrealm fo
# form a slope
order_subrealm <- analysis_data %>%
  group_by(order, subrealm) %>%
  summarize(number_species_sub_order=n()) %>%
  dplyr::filter(number_species_sub_order>=2)
...
body_size_summarized <- analysis_data %>%
  ungroup() %>%
  unite(order_sub, order, subrealm, sep="_", remove=FALSE) %>%
  dplyr::filter(order_sub %in% local(order_subrealm %>%
                                        unite(order_sub, order, subrealm, sep="_", remove=FALSE) %>%
                                        .$order_sub)) %>%
  unite(metadata2, type, units, measurement_detail, sep="_", remove=FALSE) %>%
  group_by(order, metadata2) %>%
  summarize(N=n())
...
# now summarize the data at the order level
# first filter out based on the data above regarding 2 species in  order subrealm
# number of orders with >10 species
...
order_summary <- analysis_data %>%
  ...
  dplyr::filter(order_sub %in% local(order_subrealm %>%
                                       ...
                                       .$order_sub)) %>%
  unite(metadata2, type, units, measurement_detail, sep="_", remove=FALSE) %>%
  group_by(order, class, phylum, kingdom) %>%
...
saveRDS(order_summary, "analysis_data/summary_order.RDS")
...
saveRDS(analysis_data2, "analysis_data/final_data_for_analysis_order.RDS")

analysis_data2 %>%
  group_by(metadata) %>%
  summarize(N=n()) %>%
  mutate(model_type="order_viirs") %>%
  saveRDS(., "intermediate_results/model_metadata_usage_summary/metadata_summary_order_viirs.RDS")

# now a function to model every order possible
# and save the model results out
# as a file
order_level_modelling_function <- function(order_name){
  
  message(paste0("Modelling ", order_name))
  ...
  if(unique(dat$model_type)=="type 1"){
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm) + (1|metadata2),
              ...
              file=paste0("model_objects/order_level/", order_name, "_viirs_type_1"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10,
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/order_level/", order_name, "_viirs_type_2"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/order_level/", order_name, "_viirs_type_3"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1|metadata2),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/order_level/", order_name, "_viirs_type_4"))
    
  }
}

# now apply this modelling function over every order
# but will use an lapply with error
lapply_with_error(order_summary$order, order_level_modelling_function)

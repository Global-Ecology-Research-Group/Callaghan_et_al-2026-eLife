...
family_level_modelling_function <- function(family_name){
  ...
  if(unique(dat$model_type)=="type 1"){
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm) + (1 + body_size_scaled_log10|metadata2),
              data=dat,
              warmup=1000,
              iter=6000, 
              chains=4,
              ...
              file=paste0("model_objects/family_level_v2/", family_name, "_viirs_type_1"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10,
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/family_level_v2/", family_name, "_viirs_type_2"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/family_level_v2/", family_name, "_viirs_type_3"))
    
  } else if(unique(dat$model_type)=="type 4"){
    
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|metadata2),
              data=dat,
              warmup=1000,
              iter=6000, 
              chains=4,
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/family_level_v2/", family_name, "_viirs_type_4"))

# this script is fitting models at the phylum level
# where the urban scores were calculated using the viirs night-time lights
# it mirrors the script "R/modelling/phylum_level_modelling_ghm.R"
...
# first calculate the number of phylums with >2 species in a subrealm
# this seems like a logical cutoff in that we want at least 2 species in a subrealm fo
# form a slope
phylum_subrealm <- analysis_data %>%
  group_by(phylum, subrealm) %>%
  summarize(number_species_sub_phylum=n()) %>%
  dplyr::filter(number_species_sub_phylum>=2)
...
body_size_summarized <- analysis_data %>%
  ungroup() %>%
  unite(phylum_sub, phylum, subrealm, sep="_", remove=FALSE) %>%
  dplyr::filter(phylum_sub %in% local(phylum_subrealm %>%
                                       unite(phylum_sub, phylum, subrealm, sep="_", remove=FALSE) %>%
                                       .$phylum_sub)) %>%
  unite(metadata2, type, units, measurement_detail, sep="_", remove=FALSE) %>%
  group_by(phylum, metadata2) %>%
  summarize(N=n())
...
# now summarize the data at the phylum level
# first filter out based on the data above regarding 2 species in  phylum subrealm
# number of phylumes with >10 species
...
saveRDS(phylum_summary, "analysis_data/summary_phylum.RDS")
...
saveRDS(analysis_data2, "analysis_data/final_data_for_analysis_phylum.RDS")

analysis_data2 %>%
  group_by(metadata) %>%
  summarize(N=n()) %>%
  mutate(model_type="phylum_viirs") %>%
  saveRDS(., "intermediate_results/model_metadata_usage_summary/metadata_summary_phylum_viirs.RDS")

# now a function to model every phylum possible
# and save the model results out
# as a file
phylum_level_modelling_function <- function(phylum_name){
  
  message(paste0("Modelling ", phylum_name))
  
  # filter to data
  dat <- analysis_data2 %>%
    dplyr::filter(phylum==phylum_name) %>%
  ...
  if(unique(dat$model_type)=="type 1"){
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm) + (1|metadata2),
              ...
              file=paste0("model_objects/phylum_level/", phylum_name, "_viirs_type_1"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10,
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/phylum_level/", phylum_name, "_viirs_type_2"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/phylum_level/", phylum_name, "_viirs_type_3"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1|metadata2),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/phylum_level/", phylum_name, "_viirs_type_4"))
    
  }
}

# now apply this modelling function over every phylum
# but will use an lapply with error
lapply_with_error(phylum_summary$phylum, phylum_level_modelling_function)

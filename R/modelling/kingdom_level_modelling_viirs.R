...
# it mirrors the script "R/modelling/kingdom_level_modelling_ghm.R"
...
saveRDS(kingdom_summary, "analysis_data/summary_kingdom.RDS")
...
saveRDS(analysis_data2, "analysis_data/final_data_for_analysis_kingdom.RDS")

analysis_data2 %>%
  group_by(metadata) %>%
  summarize(N=n()) %>%
  mutate(model_type="kingdom_viirs") %>%
  saveRDS(., "intermediate_results/model_metadata_usage_summary/metadata_summary_kingdom_viirs.RDS")
...
kingdom_level_modelling_function <- function(kingdom_name){
  ...
  if(unique(dat$model_type)=="type 1"){
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm) + (1|metadata2),
              ...
              file=paste0("model_objects/kingdom_level/", kingdom_name, "_viirs_type_1"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10,
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/kingdom_level/", kingdom_name, "_viirs_type_2"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1 + body_size_scaled_log10|subrealm),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/kingdom_level/", kingdom_name, "_viirs_type_3"))
    ...
    brms::brm(mean_urban_score_viirs ~ body_size_scaled_log10 + (1|metadata2),
              ...
              prior=myprior,
              control=list(adapt_delta=0.99),
              file=paste0("model_objects/kingdom_level/", kingdom_name, "_viirs_type_4"))

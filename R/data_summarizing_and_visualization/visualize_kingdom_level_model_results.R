...
# 1) R/modelling/kingdom_level_modelling.R
# 2) R/modelling/summarize_kingdom_level_model_fits.R

# read in data
dat <- readRDS("intermediate_results/kingdom_level_results.RDS")
...
dat %>%
  dplyr::select(kingdom, response_type, estimate, kingdom) %>%
  distinct() %>%
...
dat %>%
  ...
  dplyr::select(kingdom, estimate, kingdom) %>%
  distinct() %>%
...
dat %>%
  ...
  ggplot(., aes(y=fct_inorder(kingdom), x=b_body_size_scaled_log10))+
  geom_density_ridges()+
  theme_bw()+
  facet_wrap(~kingdom, scales="free")+
  xlab("Posterior")+

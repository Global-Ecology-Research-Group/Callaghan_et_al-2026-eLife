...
## summarize the model fits across the order level
...
# get the order level summary as well
...
length(unique(analysis_data2$order))


# how many species fit for the different families
sum(order_summary$number_species)
length(unique(order_summary$order))

# check something
analysis_data2 %>%
  dplyr::filter(order %in% order_summary$order) %>%
  .$species %>%
  unique() %>%
  length()
...
p_value_summary <- dat %>%
  dplyr::select(kingdom, order, fixed_p_value) %>%
  distinct() %>%

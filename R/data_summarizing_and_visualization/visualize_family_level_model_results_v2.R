...
dat_family <- readRDS("intermediate_results/family_level_results_v2.RDS") %>%
...
ggsave("Figures/small_animal_histogram_of_parameter_estimates_v2.png", width=3, height=2, units="in", bg="transparent")
...
ggsave("Figures/small_plant_histogram_of_parameter_estimates_v2.png", width=3, height=2, units="in", bg="transparent")
...
# file names
family_mods <- list.files("model_objects/family_level_v2/")
...
model_results_viz <- function(family_name){
  ...
  mod <- readRDS(paste0("model_objects/family_level_v2/", family_name, "_viirs_", dat$model_type, ".RDS"))
  ...
  plot_summary <- (wrap_elements(table) | wrap_elements(posterior)) / wrap_elements(predicted_fit) + plot_annotation(title = paste0(family_name, ", ", dat$order, ", ", dat$class))
  
  plot_summary
  
  if (dat$kingdom=="Animalia") {
    
    ggsave(paste0("Results/family_level_model_vis_v2/Animalia/", family_name, ".png"), width=6.5, height=5.8, units="in")
    
  } else {
    
    ggsave(paste0("Results/family_level_model_vis_v2/Plantae/", family_name, ".png"), width=6.5, height=5.8, units="in")

...
# 1) R/modelling/class_level_modelling.R
# 2) R/modelling/summarize_class_level_model_fits.R

# read in data
dat <- readRDS("intermediate_results/class_level_results.RDS")
...
######## fit for every class and makes a visualization of the results
######## and then saves out this visualization as an individual png
######## into the following folder: "Results/class_level_model_vis"
...
# file names
class_mods <- list.files("model_objects/class_level/")

# visualize model results function
model_results_viz <- function(class_name){
  
  message(paste0("Summarizing ", class_name))
  ...
  mod <- readRDS(paste0("model_objects/class_level/", class_name, "_viirs_", dat$model_type, ".RDS"))
  ...
  plot_summary <- (wrap_elements(table) | posterior) / predicted_fit + plot_annotation(title = paste0(class_name))
  
  plot_summary
  
  if (dat$kingdom=="Animalia") {
    
    ggsave(paste0("Results/class_level_model_vis/Animalia/", class_name, ".png"), width=6.5, height=5.8, units="in")
    
  } else {
    
    ggsave(paste0("Results/class_level_model_vis/Plantae/", class_name, ".png"), width=6.5, height=5.8, units="in")
...
lapply(class_summary$class, model_results_viz)
